  ...
  >
    ...
    >
      <image
        ...
        :webp="true"
        :src="fixCMSPath(config.content.image, 1125)"
        @load="onImageLoad($event)"
        @error="
          onLoadedError(
            { type: 'image', src: fixCMSPath(config.content.image, 1125) },
            $event
          )
        "
      />
      <view :class="['handle-area', isActive ? 'show' : '']">
        <!-- 主标题 -->
        <view class="single_image-ttile"  v-if="config.content.title" :style="titleStyles">{{
          config.content.title
        }}</view>
        <!-- 副标题 -->
        <view class="single_image-sub_ttile" v-if="config.content.sub_title" :style="subtitleStyles">{{
        ...
        >
          <block  v-for="(btn, btnIdx) in config.content.buttons"  :key="btnIdx">
            <button
              class="btn"
