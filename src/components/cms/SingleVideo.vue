  ...
  <view :class="['single_video-wrapper cms_com', isEmbedded ? 'is-embedded' : '']" :style="appearanceStyles" :id="`_${identifier}_${config.id}`" :dataCmsId="config.id" >
    <view class="single_video-container"  :style="videoStyle">
      <video
        ...
        :style="videoStyle"
        :catch-move="true"
        :src="fixCMSPath(config.content.video)" :poster="fixCMSPath(config.content.image)"
        :autoplay="isAutoPlay" :showCenterPlayBtn="false" :loop="isAutoPlay" :muted="muted" :object-fit="isEmbedded ? 'cover' : 'contain'"
        :controls="false" :show-progress="false" :show-mute-btn="false" :show-fullscreen-btn="false"
      ...
      />
      <!-- 静音按钮 -->
      <view class="video-mete" v-if="config.content.show_voice_button" :style="{ top: `${headerHeight + 40}rpx` }" @tap.stop="toggleMuted">
      ...
      <!-- 播放按钮 -->
      <view class="icon-play-box" hover-class="none" hover-stop-propagation="false"  @tap.stop="playVideo" v-show="playBtnFlag">
        <image class="icon-paly" mode="widthFix" src="@/assets/images/icons/play.svg"/>
      </view>
      <view :class="['single_video-ctas--wrapper', isActive ? 'show' : '']">
        <!-- 主标题 -->
        <view class="single_video-title" v-if="config.content.title" :style="titleStyles">{{ config.content.title }}</view>
        <!-- 副标题 -->
        <view class="single_video-sub_title" v-if="config.content.sub_title" :style="subtitleStyles">{{ config.content.sub_title }}</view>
        <!-- 按钮 -->
        <view
          v-if="Array.isArray(config.content.buttons) && config.content.buttons.length"
          class="btn_group" :style="btnGroupStyles"
        >
          <button
            v-for="(btn, idx) in config.content.buttons" :key="idx"
            class="btn" :style="toStylesForBtn(btn)" @tap.stop="doJumpToLink(btn)"
          >{{ btn.text }}</button>
        </view>
      </view>
      <!-- 下滑按钮 -->
